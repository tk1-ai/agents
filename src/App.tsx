...
type Level = 1 | 2 | 3
type AgentKey = 'email' | 'research'
type Choice = 'takeControl' | 'trustAgents'
type RunMode = 'script' | 'ai' | 'freechat'
...
interface ChatMessage { id: string; agent: 'EA' | 'RA' | 'USER' | 'SYSTEM'; name: string; text: string; time: string; level?: Level; border?: string }
...
const keyToInitials: Record<AgentKey, 'EA' | 'RA'> = { email: 'EA', research: 'RA' }
...
function buildOutcome(
  ...
  const agentSummary = `${researchSentence} ${emailSentence}`
...
export default function App() {
  ...
  const aiHistoryRef = useRef<{ agent: string; text: string }[]>([])

  // Free chat state
  const [userInput, setUserInput] = useState('')
  const [isSending, setIsSending] = useState(false)
  const abortRef = useRef<AbortController | null>(null)
  const freechatHistoryRef = useRef<{ agent: string; text: string }[]>([])
  ...
  // Fires the agent's n8n webhook for scripted scenario lines
  ...
  function runFrom(startIndex: number, msgs = scenario) {
    ...
    for (let i = startIndex; i < msgs.length; i++) {
      ...
      const t2 = window.setTimeout(() => {
        ...
        maybeFireN8N(msg, agentLevel, messageText)
  ...
  async function runAIFrom(startIndex: number) {
    for (let i = startIndex; i < aiBeats.length; i++) {
      ...
      try {
        ...
        chatLine = `(AI request failed) ${(err as Error).message}`
      }
  ...
  function startFreeChat() {
    ...
    // Welcome message
    setChatMessages([{
      id: uid(), agent: 'SYSTEM', name: 'System', time: now(),
      text: 'Free chat is live. Type a command to direct your agents. Use @EA for Email Agent, @RA for Research Agent, or just type to talk to both.',
    }])
    setTimeout(() => inputRef.current?.focus(), 300)
  }

  async function callAgent(agentKey: AgentKey, beat: string): Promise<string | null> {
    const agentLevel = (lockedLevels[agentKey] || 1) as Level
    const agentCfg = AGENTS.find(a => a.id === agentKey)!
    const initials = keyToInitials[agentKey]

    // Show typing
    const typingId = 'typing-' + uid()
    setChatMessages(prev => [...prev, { id: typingId, agent: initials, name: '', text: '__typing__', time: now(), level: agentLevel }])
    scrollToBottom()
    ...
    try {
      ...
      const res = await triggerN8NSync(url, {
        ...
        agent: agentKey,
        level: agentLevel,
        scenarioContext: 'Free chat -- the user is giving direct commands to their agents. Respond naturally, take action based on your autonomy level, and coordinate with the other agent when relevant.',
        beat,
        conversationHistory: freechatHistoryRef.current.slice(-30),
        timestamp: new Date().toISOString(),
      })
      chatLine = res.chatLine
    } catch (err) {
      chatLine = `(Request failed) ${(err as Error).message}`
    }

    if (!chatLine || !chatLine.trim()) {
      chatLine = '(Empty response from n8n -- check the workflow)'
    }

    // Remove typing, add real message
    setChatMessages(prev => prev.filter(m => m.id !== typingId))
    addMessage({ agent: initials, name: agentCfg.name, text: chatLine, level: agentLevel, border: BORDER_STYLES[agentLevel] })
    freechatHistoryRef.current = [...freechatHistoryRef.current, { agent: agentCfg.name, text: chatLine }]
    addAudit(`${agentCfg.name} responded (Level ${agentLevel})`)
  ...
  async function handleSendMessage() {
    ...
    addMessage({ agent: 'USER', name: 'You', text: raw })
    freechatHistoryRef.current = [...freechatHistoryRef.current, { agent: 'You', text: raw }]

    const lower = raw.toLowerCase()

    // Parse who to target
    let firstAgent: AgentKey
    let secondAgent: AgentKey | null = null

    if (lower.startsWith('@ea') || lower.startsWith('@email')) {
      firstAgent = 'email'
      // At level 2+, email agent might have info worth sharing with research
      const emailLevel = (lockedLevels.email || 1) as Level
      if (emailLevel >= 2) secondAgent = 'research'
    } else if (lower.startsWith('@ra') || lower.startsWith('@research')) {
      firstAgent = 'research'
      const researchLevel = (lockedLevels.research || 1) as Level
      if (researchLevel >= 2) secondAgent = 'email'
    } else {
      // Default: research first (it triages), then email follows up
      firstAgent = 'research'
      secondAgent = 'email'
    }

    // Clean the @ prefix from the message
    const cleanMessage = raw.replace(/^@\w+\s*/i, '') || raw

    // Call the first agent
    const firstResponse = await callAgent(firstAgent, cleanMessage)

    // If the first agent responded and we should chain, call the second agent
    if (firstResponse && secondAgent) {
      // Small pause so user can read the first response
      await new Promise(r => setTimeout(r, 1000))

      const firstAgentName = AGENTS.find(a => a.id === firstAgent)!.name
      const secondBeat = `The user asked: "${cleanMessage}". ${firstAgentName} responded: "${firstResponse}". Now respond from your perspective -- what would you add, do, or follow up on?`

      await callAgent(secondAgent, secondBeat)
    }
  ...
  function handleStopChat() {
    setIsSending(false)
    setChatMessages(prev => prev.filter(m => m.text !== '__typing__'))
    addMessage({ agent: 'SYSTEM', name: 'System', text: 'Stopped. Your agents are standing by.' })
  }
  ...
  function handleShare() {
    ...
    const text = `I scored ${capabilityScore}/6 on the Agent Directive Console. Outcome: ${oc?.title || ''}. Try it: ${window.location.href}`
    navigator.clipboard.writeText(text).then(() => alert('Result copied to clipboard!'))
  }

  const progressPct = (configuredCount / 2) * 100
          ...
          <div className="slide-up" style={{ background: C.white, borderRadius: 14, padding: 20, boxShadow: '0 1px 3px #0001', border: `1px solid ${C.border}`, marginBottom: 24 }}>
            ...
            <div ref={chatRef} style={{ height: 380, overflowY: 'auto', background: C.bg, borderRadius: 10, padding: 14, marginBottom: 12, display: 'flex', flexDirection: 'column', gap: 10, border: `1px solid ${C.border}` }}>
              {chatMessages.map(msg => {
                ...
                if (msg.agent === 'SYSTEM') {
                  return (
                    <div key={msg.id} className="fade-in" style={{ textAlign: 'center', padding: '10px 16px', background: C.white, borderRadius: 10, border: msg.border || `1px solid ${C.border}`, fontSize: 12, color: C.charcoal, fontStyle: 'italic' }}>
                      {msg.text}
                    </div>
                  )
                }
                if (msg.agent === 'USER') {
                  return (
                    <div key={msg.id} className="fade-in" style={{ display: 'flex', gap: 9, alignItems: 'flex-start', flexDirection: 'row-reverse' }}>
                      <div style={{ width: 30, height: 30, borderRadius: '50%', background: C.black, color: C.white, display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, fontSize: 10, flexShrink: 0 }}>
                        YOU
                      </div>
                      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'flex-end' }}>
                        ...
                        <div style={{ background: C.black, color: C.white, borderRadius: '10px 0 10px 10px', padding: '9px 13px', fontSize: 12, lineHeight: 1.6, maxWidth: '85%' }}>
                          {msg.text}
                        </div>
                      </div>
                    </div>
              ...
              {runMode === 'freechat' && chatMessages.length <= 1 && !isSending && (
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, justifyContent: 'center', padding: '12px 0' }}>
                  {[
                    ...
                    '@EA Draft a follow-up email to the CFO',
                    'What do we know about Vantage Solutions?',
                  ].map(s => (
                    <button key={s} onClick={() => { setUserInput(s); inputRef.current?.focus() }}
                      style={{ padding: '6px 12px', borderRadius: 20, border: `1px solid ${C.border}`, background: C.white, fontSize: 11, color: C.grey, cursor: 'pointer' }}>
                      {s}
                    </button>
                  ))}
                </div>
            ...
            {runMode === 'freechat' && (
              <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
                <input
                  ref={inputRef}
                  type="text"
                  placeholder="Tell your agents what to do... (@EA, @RA, or both)"
                  value={userInput}
                  ...
                  style={{
                    ...
                    background: isSending ? C.bg : C.white, color: C.charcoal,
                  }}
                />
                {isSending ? (
                  <button onClick={handleStopChat}
                    style={{ padding: '10px 16px', borderRadius: 10, border: `2px solid #cc3333`, background: C.white, color: '#cc3333', fontWeight: 700, fontSize: 12, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
                    <Square size={12} /> Stop
                  </button>
                ) : (
                  <button onClick={handleSendMessage} disabled={!userInput.trim()}
                    style={{
                      ...
                      background: userInput.trim() ? C.black : C.border,
                      color: userInput.trim() ? C.white : C.greyLight,
